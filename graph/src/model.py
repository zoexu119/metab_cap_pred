...
class MLNet2_Encoder(torch.nn.Module):
    def __init__(self, num_node_features, num_edge_features, num_global_features, hidden, dropout, depth, graph_level_feature):
        super(MLNet2_Encoder, self).__init__()
        self.mlp_node = MLP(num_node_features, hidden, hidden, dropout, F.relu)
        self.mlp_edge = MLP(num_edge_features, hidden, hidden, dropout, F.relu)
        self.emb_subgraph = torch.nn.Embedding(4, hidden)
        self.graph_level_feature = graph_level_feature
...
class MLN2Target(torch.nn.Module):
    def __init__(self, num_node_features, num_edge_features, num_global_features, hidden_enc, hidden_pred, dropout, num_tasks, depth, graph_level_feature):
        super(MLN2Target, self).__init__()

        self.encoder = MLNet2_Encoder(num_node_features, num_edge_features, num_global_features, hidden_enc, dropout, depth, graph_level_feature)
        if graph_level_feature:
            self.mlp1 = MLP(num_global_features+hidden_enc, hidden_pred, num_tasks, dropout, F.relu)
        else:
            self.mlp1 = MLP(hidden_enc, hidden_pred, num_tasks, dropout, F.relu)

    def forward(self, batch_data):
        encoded_x, encoded_u = self.encoder(batch_data)
        out = self.mlp1(encoded_u)
        return out
