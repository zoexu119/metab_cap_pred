...
# from loss_utils import *#loss_utils as loss_utils, precision
...
class rank_loss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        if self.weights is not None:
            ...
            weights = y_true * back_weights + (1.0 - y_true) * fore_weights
        else:
            weights = torch.ones(y_true.shape)
            if self.use_gpu:
                weights = weights.to('cuda')

        true_id = torch.nonzero(y_true)[:,0]
        false_id = torch.nonzero(1-y_true)[:,0]

        true_pred = y_pred[true_id]
        false_pred = y_pred[false_id]

        lossfunc = torch.nn.MarginRankingLoss()
        loss = 0

        for positive in true_pred:
            for negative in false_pred:
                loss += lossfunc(positive,negative,torch.tensor([1]).to('cuda'))
...
class hinge_loss(nn.Module):
    ...
    def forward(self, outputs, label):
        probs = outputs
        label = 2 * label - 1.0
        loss = torch.mean(1.0 - label * probs)
        return loss
